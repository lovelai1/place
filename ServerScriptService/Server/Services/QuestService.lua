...
local magicTierService: any = nil  -- Phase 5: AMP on quest turn-in
...
local function distributeRewards(player: Player, questId: string, data: { [string]: any }): { [string]: any }
	...
	-- Gold: direct mutation
	data.Gold = (data.Gold or 0) + reward.gold

	-- EXP: via LevelService so level-up fires; fallback to raw add
	if levelService and reward.exp and reward.exp > 0 then
		pcall(levelService.AddExp, player, reward.exp)
	else
		data.Exp = (data.Exp or 0) + (reward.exp or 0)
	end
	...
	-- Phase 5: grant AMP scaled to quest EXP
	if magicTierService and reward.exp and reward.exp > 0 then
		local ampGrant = math.clamp(math.floor(reward.exp / 5), 50, 400)
		pcall(magicTierService.AddAMP, player, ampGrant, "QuestReward")
	end
...
-- ── public read-only checks (used by NPCService to build dialog payload) ──

function QuestService.CanAccept(player: Player, questId: string): boolean
...
function QuestService.CanTurnIn(player: Player, questId: string): boolean
	local data = getData(player)
	if not data then return false end
	local active = getActiveQuests(data)
...
function QuestService.Init(newDataService: any, newInventoryService: any?, newLevelService: any?, newMagicTierService: any?)
